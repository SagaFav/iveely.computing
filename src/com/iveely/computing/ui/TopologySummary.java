...
public class TopologySummary {
	...
	public static class TopologySimple {
		...
		public void init() {
			...
			int finishCount = ZookeeperClient.getInstance().getChildren("/app/" + this.name + "/finished").size();
			...
			this.inSlaveCount = Integer
					.parseInt(ZookeeperClient.getInstance().getNodeValue("/app/" + this.name + "/slavecount"));
