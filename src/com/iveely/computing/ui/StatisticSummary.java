...
public class StatisticSummary {
	...
	public void init(String tpName) {
		...
		taskType.stream().forEach((String tt) -> {
			...
			allClasses.stream()
					.map((clas) -> ZookeeperClient.getInstance().getChildren("/app/" + tpName + "/" + tt + "/" + clas))
			...
			inputOrOutputs.stream().map((ioput) -> {
				...
				// 2.1 emit
				String val = ZookeeperClient.getInstance()
						.getNodeValue("/app/" + tpName + "/statistic/" + tt + "/" + ioput + "/emit");
				if (val.isEmpty()) {
				...
				// 2.2 emit avg
				val = ZookeeperClient.getInstance()
						.getNodeValue("/app/" + tpName + "/statistic/" + tt + "/" + ioput + "/emit_avg");
				if (val.isEmpty()) {
					detail.setEmitAvg("0");
				} else {
				...
				// 2.3 execute
				val = ZookeeperClient.getInstance()
						.getNodeValue("/app/" + tpName + "/statistic/" + tt + "/" + ioput + "/execute");
				if (val.isEmpty()) {
				...
				// 2.4 execute avg
				val = ZookeeperClient.getInstance()
						.getNodeValue("/app/" + tpName + "/statistic/" + tt + "/" + ioput + "/execute_avg");
				if (val.isEmpty()) {
