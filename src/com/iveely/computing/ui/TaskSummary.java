	...
	public void init(String tpName) {
		...
		taskType.stream().forEach((tt) -> {
			...
			inputOrOutputs.stream().map((inputOrOutput) -> {
				...
				task.setType(tt);
				String connect = ZookeeperClient.getInstance()
						.getNodeValue("/app/" + tpName + "/" + tt + "/" + inputOrOutput);
				task.setConnectPath(connect);
				List<String> exceptionSlots = ZookeeperClient.getInstance()
						.getChildren("/app/" + tpName + "/status/" + tt + "/" + simDetail[1]);
				if (exceptionSlots.isEmpty()) {
					task.setException(" ");
				} else {
					String excpt = ZookeeperClient.getInstance().getNodeValue(
							"/app/" + tpName + "/status/" + tt + "/" + simDetail[1] + "/" + exceptionSlots.get(0));
