...
public class OutputExecutor extends IExecutor implements Runnable {
    ...
    public OutputExecutor(String name, IOutput output, HashMap<String, Object> conf) {
        ...
        ZookeeperClient.getInstance().setNodeValue(
                "/app/" + this._name + "/output/" + output.getClass().getName() + "/" + output.getName(), connectPath);
    }
    ...
    public void invoke(Tuple tuple) {
        try {
            ...
            if (this._executeCount % 100 == 0) {
                ...
                ZookeeperClient.getInstance().setNodeValue(
                        "/app/" + this._name + "/statistic/output/" + this._output.getName() + "/execute",
                        this._executeCount + "");
                ZookeeperClient.getInstance().setNodeValue(
                        "/app/" + this._name + "/statistic/output/" + this._output.getName() + "/execute_avg",
            ...
            ZookeeperClient.getInstance().setNodeValue("/app/" + this._name + "/status/output/" + this._output.getName()
                    + "/" + this._slot.getConnectString(), error.toString());
            ZookeeperClient.getInstance().setNodeValue("/app/" + this._name + "/finished", -1 + "");
            ZookeeperClient.getInstance().setNodeValue("/app/" + this._name + "/finished/" + this._output.getName(),
                    "Exception stopped.");
        }
    }
